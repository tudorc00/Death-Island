...
namespace BlazeAISpace
{
    public class NormalStateBehaviour : MonoBehaviour
    {
        ...
        void OnEnable() 
        {
            ...
            // if forced to change state from alert to normal using ChangeState()
            if (blaze.changedState) {
                blaze.changedState = false;
                return;
            }

            // if method used by user to move to a certain location -> don't set next way point
            if (!blaze.movedToLocation && !blaze.waypoints.randomize) blaze.NextWayPoint();
        }
        ...
        void SetIdleState()
        {
            // tell blaze whether this behaviour is isdle or not
            blaze.isIdle = isIdle;
        }
