...
namespace KnifePlayerController
{
    public class Grenade : MonoBehaviour
    {
        ...
        {
            if(waitCasts)
                grenadeSphereRaycastCommand.WaitJobComplete();
        }
        ...
        private void raycasted(NativeArray<RaycastHit> raycastHits)
        {
            RaycastHit[] hits;
            ...
            foreach(Collider c in targets)
            {
                ...
                if(hittable != null && !damagedTargets.Contains(hittable))
                {
                    ...
                    if(c.bounds.Contains(transform.position) || !Physics.Raycast(r, out hitInfo, direction.magnitude, CollisionMask))
                    {
                        ...
                        //Debug.Log("HIT NO COLLISION: " + c.name + " " + distance + " " + value);
                        //Debug.DrawLine(transform.position, r.origin + direction, Color.red, 15f);
                    }
                    else
                    {
                        if (hitInfo.collider == c)
                        {
                            ...
                            hittable.TakeDamage(damage);
                            //Debug.Log("HIT COLLISION TO TARGET: " + c.name + " " + distance + " " + value);
                            //Debug.DrawLine(transform.position, hitInfo.point, Color.red, 15f);
                        }
                        else
                        {
                           // Debug.Log("Potential: " + c.name + " collision: " + hitInfo.collider.name);
                        }
                    }
                    damagedTargets.Add(hittable);
                }
